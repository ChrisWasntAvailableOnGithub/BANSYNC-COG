"""Created By ChrisWasntAvailable for use on the Apollo Discord bot
Latest Update 01/08/2023 1500 BST"""
...
async def create_case(
    ...
    await _red_create_case(bot, guild, created_at, action_type, uid, moderator, reason)
...
class BanSync(commands.Cog):
    ...
    async def importbans(self, ctx: commands.GuildContext):
        ...
        current_bans = []
        async for b in ctx.guild.bans():
            current_bans.append(b.user.id)
        to_ban = set(data) - set(current_bans)
    ...
    async def ban_or_hackban(
        ...
        *,
        ...
        if not await self.ban_filter(guild, mod, member):
            return False
        try:
            await guild.ban(member, reason=audit_reason, delete_message_days=7)
        except discord.HTTPException:
            return False
        else:
            await create_case(
                self.bot, guild, datetime.utcnow(), "ban", member, mod, audit_reason
            )
            return True
    ...
    async def mjolnir(self, ctx, users: Greedy[MentionOrID], *, reason: str = "None Provided"):
        ...
        if any(banned):
            await ctx.send("Global Ban Complete. Houston, be advised the visual is GO today. #PermBan")
        else:
            await ctx.send("You are not worthy")
